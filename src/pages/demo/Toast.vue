<template>
  <demo title="Icon" class="icon-demo">
    <demo-item name="default" description="默认" :code="size_code" slot="left">
      <Btn @click="$toast(nowDate)">Toast</Btn>
    </demo-item>

    <demo-item class="type-btn-group" name="type" description="主题" :code="type_code" slot="right">
    ...
    <demo-item name="time" description="自动关闭的延时，单位毫秒" :code="time_code" slot="left">
      <Btn @click="$toast('一秒后消失', { time: 1000 })">延迟一秒</Btn>
      <Btn @click="control">手动控制</Btn>
    </demo-item>

    <demo-item name="callback" description="消失后的回调" :code="callback_code" slot="right">
